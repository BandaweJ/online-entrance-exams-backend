...
export class ExamScoringService {
  ...
  async scoreExam(attemptId: string): Promise<{
    ...
    // Calculate totals
    const totalScore = gradedAnswers.reduce((sum, answer) => sum + parseFloat(answer.score.toString()), 0);
    const totalMarks = gradedAnswers.reduce((sum, answer) => sum + parseFloat(answer.maxScore.toString()), 0);
  ...
  private scoreObjectiveQuestion(answer: Answer, question: Question): {
    score: number;
    feedback: string;
    isCorrect: boolean;
  } {
    const correctAnswer = question.correctAnswer;
    const studentAnswer = answer.answerText || answer.selectedOptions?.join(', ') || '';
  ...
  private async scoreSubjectiveQuestion(answer: Answer, question: Question): Promise<{
    score: number;
    feedback: string;
  }> {
    const correctAnswer = question.correctAnswer;
  ...
  private fallbackSubjectiveGrading(correctAnswer: string, studentAnswer: string, totalMarks: number): {
    score: number;
    feedback: string;
  } {
    // Simple keyword-based grading as fallback
    const correctKeywords = correctAnswer.toLowerCase().split(/\s+/).filter(word => word.length > 2);
    const studentKeywords = studentAnswer.toLowerCase().split(/\s+/).filter(word => word.length > 2);
  ...
  async getScoringProgress(attemptId: string): Promise<{
    ...
    const progressPercentage = answers.length > 0 ? (gradedAnswers / answers.length) * 100 : 0;
