...
export class AnswersService {
  ...
  async create(
    ...
    const { questionId, attemptId } = createAnswerDto;
    ...
    if (attempt.status !== AttemptStatus.IN_PROGRESS) {
      throw new BadRequestException(
        "Cannot submit answers to inactive attempt",
      );
    }
    ...
    if (question.section.exam.id !== attempt.examId) {
      throw new BadRequestException(
        "Question does not belong to the same exam",
      );
    }
  ...
  async update(
    ...
    if (answer.attempt.status !== AttemptStatus.IN_PROGRESS) {
      throw new BadRequestException(
        "Cannot update answers for inactive attempt",
      );
    }
  ...
  async remove(id: string, studentId: string): Promise<void> {
    ...
    if (answer.attempt.status !== AttemptStatus.IN_PROGRESS) {
      throw new BadRequestException(
        "Cannot delete answers for inactive attempt",
      );
    }
  ...
  async getAnswerStats(attemptId: string, studentId: string) {
    ...
    const stats = {
      ...
      totalMarks: answers.reduce(
        (sum, a) => sum + parseFloat(a.maxScore.toString()),
        0,
      ),
      byQuestionType: {
        multipleChoice: answers.filter(
          (a) => a.question.type === "multiple_choice",
        ).length,
        trueFalse: answers.filter((a) => a.question.type === "true_false")
          .length,
        shortAnswer: answers.filter((a) => a.question.type === "short_answer")
          .length,
  ...
  async getUnansweredQuestions(attemptId: string, studentId: string) {
    ...
    const allQuestions = attempt.exam.sections.flatMap(
