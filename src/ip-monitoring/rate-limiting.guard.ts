...
export class RateLimitingGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const rateLimitStatus = await this.rateLimitingService.checkRateLimit(
      ipAddress,
      action,
    );
    ...
    if (!rateLimitStatus.allowed) {
      const rateLimitHeaders =
        await this.rateLimitingService.getRateLimitHeaders(ipAddress, action);
      ...
      throw new HttpException(
        {
          ...
          retryAfter: Math.ceil(
            (rateLimitStatus.resetTime.getTime() - Date.now()) / 1000,
          ),
        },
    ...
    const response = context.switchToHttp().getResponse();
    const rateLimitHeaders = await this.rateLimitingService.getRateLimitHeaders(
      ipAddress,
      action,
    );
    Object.entries(rateLimitHeaders).forEach(([key, value]) => {
