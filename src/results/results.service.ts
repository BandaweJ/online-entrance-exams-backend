...
export class ResultsService {
  ...
  async generateResult(attemptId: string, studentId: string): Promise<Result> {
    ...
    if (attempt.status !== AttemptStatus.SUBMITTED) {
      throw new BadRequestException('Cannot generate result for non-submitted attempt');
    }
    ...
    const questionResults = await this.generateQuestionResults(attempt, gradedAnswers);
  ...
  async findOne(id: string): Promise<Result> {
    const result = await this.resultRepository.findOne({
      where: { id },
      relations: ['student', 'exam', 'attempt', 'attempt.answers', 'attempt.answers.question'],
    });
    ...
    if (result.attempt && result.attempt.answers) {
      const gradedAnswers = result.attempt.answers.filter(answer => answer.isGraded);
      const questionResults = await this.generateQuestionResults(result.attempt, gradedAnswers);
  ...
  async getExamStats(examId: string) {
    ...
    const stats = {
      ...
      averagePercentage: percentages.reduce((sum, percentage) => sum + percentage, 0) / percentages.length,
  ...
  async getStudentStats(studentId: string) {
    ...
    if (results.length > 0) {
      ...
      stats.averagePercentage = results.reduce((sum, r) => sum + r.percentage, 0) / results.length;
  ...
  private async generateQuestionResults(attempt: ExamAttempt, gradedAnswers: Answer[]): Promise<any[]> {
