...
export class ExamsService {
  ...
  async getStats() {
    ...
    const draftExams = await this.examRepository.count({ where: { status: ExamStatus.DRAFT } });
    const publishedExams = await this.examRepository.count({ where: { status: ExamStatus.PUBLISHED } });
    const closedExams = await this.examRepository.count({ where: { status: ExamStatus.CLOSED } });
